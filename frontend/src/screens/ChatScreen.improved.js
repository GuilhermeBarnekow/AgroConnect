...
import { 
  ...
  getMessages, 
  markMessagesAsRead 
} from '../services/chatService.improved'; 
import { db } from '../services/firebase.improved'; // Import Firebase for compatibility
  ...
  useEffect(() => {
    const initializeChat = async () => {
      try {
        ...
        const chat = await createChat(user.id, otherUserId, announcementId, announcementTitle || title);
        if (chat.error) {
          console.error('Error creating chat:', chat.error);
          Alert.alert('Erro', chat.error);
          setIsLoading(false);
          return;
        }
        ...
        unsubscribeRef.current = getMessages(chat.id).then(fetchedMessages => {
